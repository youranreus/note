...
- [x] Task 1: 为在线便签补齐“作者身份”持久化边界 (AC: 1, 2, 3)
  - [x] 在 `apps/api/prisma/schema.prisma` 中补齐 `User` 模型与 `Note.author` 关系，保持 `notes.author_id -> users.id` 的内部主键关联，不要把外部 SSO id 直接当成长期外键。
  - [x] 若当前 API 侧仍只持有 `AuthUserDto.id: string`，在服务层或 repository 边界完成 `ssoId(string) -> users.sso_id(BigInt)` 的显式映射，不要把这个转换散落到 route handler 或前端。
  - [x] 在首次保存新在线便签且当前会话为 authenticated 时，为对象绑定作者；已存在对象若已有作者，不得被后续请求静默改写作者归属。
  - [x] 保持 `notes.sid` 唯一约束与 `author_id` 索引语义不变，不要因为引入作者关系破坏 Story 1.4 已经稳定的唯一对象写入路径。

- [x] Task 2: 在 notes 服务层落地“创建者默认编辑权” (AC: 1, 2, 3)
  - [x] 扩展 `note-write-service`，让写入决策接收服务端会话上下文，而不是只接收 `sid + content`。
  - [x] 对“authenticated 首次创建”与“authenticated owner 再次更新”走成功分支；对“owner-bound note + 无会话 / 错会话”返回稳定权限错误，不得继续写入。
  - [x] 明确当前 story 的边界：只实现“创建者默认编辑权”，不要提前吞并编辑密钥共享编辑逻辑；后续 Story 2.3 再接 `key_hash` 分支。
  - [x] 不要把前端传来的 user id、owner 标记或隐藏字段作为授权依据；所有 owner 判断必须从服务端 session 与数据库记录推导。

- [x] Task 3: 为读取链路补齐最小 owner 语义，支撑前后端一致校验 (AC: 1, 2, 3)
  - [x] 扩展 `GET /api/notes/:sid` 的 DTO 或等价 detail 响应，补充最小可扩展的编辑能力字段，避免只靠临时布尔值 `isOwner`。
  - [x] 当前 story 只需要覆盖最小集合：`owner-editable`、`anonymous-editable`、`forbidden` 或等价可扩展状态；不要在 2.2 提前把编辑密钥状态塞进 DTO。
  - [x] 读取链路对 owner-bound note 应结合当前 session 给出一致能力判断，让前端不会继续显示“可编辑”而服务端实际拒绝。
  - [x] 继续保持未登录阅读能力，不要因为 owner 语义引入而把 `GET /api/notes/:sid` 变成登录前置接口。

- [x] Task 4: 在 Web 端把在线便签页从“权限待接入”推进到最小 owner 语义 (AC: 1, 2, 3)
  - [x] 扩展 `packages/shared-types`、`note-methods.ts`、`use-online-note.ts` 与 `online-note.ts`，让前端状态机能消费服务端返回的 owner/edit 能力信息。
  - [x] `OnlineNoteShell.vue` 与 `NoteObjectHeader.vue` 至少要能表达：当前用户是创建者可继续编辑、当前对象为匿名/未绑定作者、当前账户不能编辑 owner-bound note。
  - [x] 对 owner-bound 但当前会话无效或身份不匹配的对象，不要继续保持“当前可继续编辑 + 保存更新”假象；至少要把保存动作禁用或在明确受控状态下提示下一步。
  - [x] 保持 2.2 的 scope：这里只补 owner 语义，不把完整查看权/编辑权矩阵、密钥输入流或用户中心动作一起做掉。

- [x] Task 5: 保持 Epic 1 与 Story 2.1 既有能力不回归 (AC: 1, 2, 3)
  - [x] 匿名用户访问首页、在线阅读页、本地便签页的路径不能被登录前置阻断。
  - [x] Story 1.4 的 `created / updated` 保存结果、Story 1.5 的复制反馈与对象头部结构、Story 2.1 的登录回跳恢复，不应因为 owner 接入被破坏。
  - [x] 仍然遵守“Pinia 只管理 auth / UI、Alova 管远端状态、权限逻辑服务端收口”的边界。
  - [x] 不要提前实现“我的创建 / 我的收藏”、收藏动作、删除动作或编辑密钥校验。

- [x] Task 6: 为 owner 默认编辑权补齐 API / Web / 状态机回归测试 (AC: 1, 2, 3)
  - [x] API 测试至少覆盖：authenticated 首次保存绑定作者、同一 owner 再次更新成功、错会话更新失败、会话失效更新失败、匿名阅读 owner-bound note 仍可读。
  - [x] Web 测试至少覆盖：owner 状态显示为可编辑、非 owner 状态不再显示误导性编辑语义、权限失败反馈有清楚文字、登录恢复后 owner 语义可重新生效。
  - [x] 状态机测试至少覆盖：sid 切换后旧保存结果不污染新对象、权限失败不会清空当前草稿、owner 语义不会破坏 Story 1.5 已有复制反馈逻辑。
  - [x] 至少执行 `pnpm --filter @note/api test`、`pnpm --filter @note/web test`、`pnpm --filter @note/api typecheck`、`pnpm --filter @note/web typecheck`；若修改 Prisma schema 或构建契约，再补跑对应 `build` 与 `db:init` 验证。

### Review Findings

- [x] [Review][Patch] 会话失效后的 owner 对象不会切换到受控只读态 [apps/web/src/features/note/use-online-note.ts:197]
- [x] [Review][Patch] 非 owner 页面仍显示“可持续更新”标签，和只读语义冲突 [apps/web/src/features/note/components/OnlineNoteShell.vue:65]
...
- 2026-04-05: 完成 owner 默认编辑权实现，补齐 `users` / `notes.author` 数据建模、读写链路 `editAccess` 契约、前端受控只读态与 API/Web 回归测试，并验证 typecheck、build 与 `db:init`。
- 2026-04-05: 已根据 code review 修复 forbidden save 不降级只读态与 non-owner badge 误导文案问题，补齐 Web 回归测试后将 story 关闭为 `done`。
...
- 在 API 侧先补齐 `users` 映射与 `notes.author` 关系，把 `AuthUserDto.id(string)` 统一在 service/repository 边界转换成 `users.sso_id -> users.id`，并把 owner 绑定、owner 校验、匿名回退都收口到 notes 读写服务。
- 在 shared types 和 notes route 上补齐最小可扩展的 `editAccess` 契约，让 `GET/PUT /api/notes/:sid` 对 owner / anonymous / forbidden 三种能力给出一致语义，同时为写入增加稳定的 `NOTE_FORBIDDEN` 错误。
- 在 Web 端扩展 `online-note` 适配器与 `use-online-note` 状态机，把 owner-bound non-owner 对象降级成“可读不可写”的受控状态，禁用保存并给出明确下一步提示。
- 以 API route/service 测试、Web 组件测试、状态机测试和全量回归命令作为完成门槛，再把 story 与 sprint 产物同步到 `review`。
...
- 已新增 `prisma-client.ts` / `user-service.ts`，把 Prisma client 初始化与 `sso_id -> users.id` 映射从 notes 读写逻辑里抽离，避免 route 层散落身份转换。
- 已在 notes read/write route、shared DTO 与前端状态机间打通 `editAccess`，并把 non-owner 场景收口为“可读不可写 + 明确提示”的受控界面，而不是继续显示假可编辑态。
- 已执行并通过 `pnpm --filter @note/api test`、`pnpm --filter @note/web test`、`pnpm --filter @note/api typecheck`、`pnpm --filter @note/web typecheck`、`pnpm --filter @note/api build`、`pnpm --filter @note/web build`。
- 已执行 `pnpm --filter @note/api db:init`；首次因 Prisma 试图重写现有 `idx_author_updated` 失败，随后将 schema 索引定义对齐到现库可落地形式后验证通过。

### Completion Notes List

- 已落地 `User` / `Note.author` Prisma 关系、`users.sso_id` 映射仓储以及 owner 首次保存绑定逻辑，确保作者归属只在 authenticated 首次创建时建立，后续不会静默改写。
- 已让 `GET /api/notes/:sid` 与 `PUT /api/notes/:sid` 统一返回 `editAccess`，并在 owner-bound note 的无会话 / 错会话更新场景返回稳定 `NOTE_FORBIDDEN` 语义。
- 已把在线便签页的对象头部、说明文案、保存按钮与编辑框状态升级为 owner-aware：创建者可编辑、匿名对象可编辑、非 owner 仅可查看且收到下一步提示。
- 已补齐并通过 API/service、Web 组件、状态机与全量回归测试；同时完成 API/Web typecheck、build 以及 Prisma `db:init` 验证。
...
- `apps/api/prisma/schema.prisma`
- `apps/api/src/routes/notes.ts`
- `apps/api/src/services/prisma-client.ts`
- `apps/api/src/services/note-read-service.ts`
- `apps/api/src/services/note-write-service.ts`
