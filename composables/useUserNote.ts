...
const useStore = defineStore('user-note', () => {
  ...
  const load = async () => {
    ...
    loading.value = true
    const res = await get<PaginationRes<MemoRes>>('/api/user/notes', {
  ...
  persist: process.client && { storage: persistedState.localStorage, key: 'user-note' },
})
