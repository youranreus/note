...
import { getUserFavourNote } from "../database/repos/noteRepo"
...
export default defineEventHandler(async (e) => {
  ...
  try {
    ...
    const [data, meta] = await getUserFavourNote(userData.data.id, Number(page ?? 1), Number(limit ?? 100))

    return {
      total: meta.totalCount,
      data: data.map((n) => transformNote(n, userData.data.id)),
    }
  } catch (error: any) {
