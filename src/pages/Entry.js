...
function Entry() {
    ...
    useEffect(() => {
        ...
        setLocalArr(localStorage.getItem('localArr').split(",").filter((item) => item !== ''))
    ...
    const addOnline = () => {
        ...
        axios.get('https://i.exia.xyz/note/get/' + newNoteId + '?key=' + newNote.key)
            .then(data => {
                if (data.data.msg)
            ...
            >
                <div style={{textAlign: "center"}}>
                    ...
                    <Collapsible isOpen={findFlag}>
                        <div onKeyDown={e => {if (e.keyCode === 13) jump()}}>
                            <InputGroup>
                                <Select defaultValue='local' onSelect={(value) => {setJumpMode(value)}}>
                                ...
                                <AutoComplete
                                    ...
                                    onChange={(v) => {setJumpId(v)}}
                                />
                            </InputGroup>
                            <div style={{textAlign: "right", marginTop: "1rem"}}>
                                <Button type={"primary"} onClick={jump}>找一下</Button>
                            </div>
                        </div>
                    </Collapsible>
                    <Collapsible isOpen={addFlag}>
                        <div className={"addNote"}>
                            ...
                            <Collapsible isOpen={newNote.mode === "online"}>
                                ...
                                <Input placeholder={"密钥（留空则无加密）"} value={newNote.key}
                                       onChange={v => {
                                           setNewNote({
                                               key: v,
                                               mode: 'online',
                            ...
                            <div style={{textAlign: "right", marginTop: "1rem"}}>
                                <Button type={"primary"} onClick={add}>撕一张</Button>
                            </div>
                        </div>
                    </Collapsible>
                </div>
